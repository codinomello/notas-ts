function calcular() {
    ...
    const campos = {
      PD: parseFloat(document.getElementById('PD')?.value ?? '0') || 0,
      NP: parseFloat(document.getElementById('NP')?.value ?? '0') || 0,
      SI: parseFloat(document.getElementById('SI')?.value ?? '0') || 0,
      PO1: parseFloat(document.getElementById('PO1')?.value ?? '0') || 0,
      PO2: parseFloat(document.getElementById('PO2')?.value ?? '0') || 0,
      PV: parseFloat(document.getElementById('PV')?.value ?? '0') || 0,
      DS: parseFloat(document.getElementById('DS')?.value ?? '0') || 0,
      EX: parseFloat(document.getElementById('EX')?.value ?? '0') || 0,
      PE: parseFloat(document.getElementById('PE')?.value ?? '0') || 0,
      MP: parseFloat(document.getElementById('MP')?.value ?? '0') || 0,
      PR: parseFloat(document.getElementById('PR')?.value ?? '0') || 0,
      ED: parseFloat(document.getElementById('ED')?.value ?? '0') || 0,
    }
    ...
    switch (nível) {
      case 'EFI':
        nota = campos.NP * 0.30 + campos.PD * 0.40 + campos.SI * 0.10
        recuperação = (nota + (campos.ED * 0.30 + campos.PR * 0.70)) / 2
        break
      case 'EFII':
        nota = campos.NP * 0.30 + campos.PD * 0.40 + campos.SI * 0.10 + ((campos.PO1 + campos.PO2 + campos.SI) * 0.30 / 2)
        recuperação = (nota + (campos.ED * 0.30 + campos.PR * 0.70)) / 2
        break
      case 'EM':
        nota = campos.NP * 0.30 + campos.PD * 0.40 + ((campos.PO1 + campos.PO2 + campos.SI) * 0.30 / 3)
    ...
    if (document.getElementById('regular').classList.contains('selecionado')) {
      if (nota === 0) {
        ...
        resultado.classList.remove('bg-teal-500')
        const abaixo = 6 - nota
        mensagem += `Quase! Você não passou. <br>Sua nota está <b>${abaixo.toFixed(1)}</b> pontos abaixo da média.`
      } else {
  ...
  function mostrar() {
    ...
    if (nível === "EFI") {
      ...
      document.getElementById('projeto-seção').classList.add('hidden')
      document.getElementById('ações').classList.add('hidden')
      document.getElementById('prova-objetiva').classList.remove('hidden')
      const PO1 = document.getElementById('PO1-label')
      PO1.innerHTML = "Prova Objetiva 1 (15%)"
      document.getElementById('PO1-label').classList.remove('hidden')
      const PO2 = document.getElementById('PO2-label')
      PO2.innerHTML = "Prova Objetiva 2 (15%)"
      document.getElementById('PO2-label').classList.remove('hidden')
    } 
    else if (nível === "EFII") {
      document.getElementById('formulário').classList.remove('hidden')
      document.getElementById('regular-seção').classList.add('hidden')
      ...
      document.getElementById('prova-objetiva').classList.remove('hidden')
      const PO1 = document.getElementById('PO1-label')
      PO1.innerHTML = "Prova Objetiva 1 (10%)"
      PO1.classList.remove('hidden')
      const PO2 = document.getElementById('PO2-label')
      PO2.innerHTML = "Prova Objetiva 2 (10%)"
  ...
  function menu(display) {
    switch (display) {
      ...
      case 4:
        ...
        document.getElementById('técnico-seção').classList.add('hidden')
        document.getElementById('recuperação-seção').classList.add('hidden')
        document.getElementById('projeto-seção').classList.remove('hidden')
        document.getElementById('ações').classList.remove('hidden')
        break;
      default:
  ...
  function remover() {
    const campos = ['PD', 'NP', 'SI', 'PO1', 'PO2', 'PV', 'DS', 'EX', 'PE', 'MP', 'PR', 'ED']
    ...
    const select = {PO1: document.getElementById('PO1'), PO2: document.getElementById('PO2')}
    if (select) {
